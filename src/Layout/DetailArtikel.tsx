import { getArtikel } from "@/Api/Mobil";
import {
  Calendar,
  Eye,
  ArrowLeft,
...
export default function DetailArtikel(): JSX.Element {
  ...
  const getDataArtikelByID = async () => {
    try {
      const respons = await getArtikel(id);
      ...
      <div className="border-b bg-white shadow-sm">
        <div className="mx-auto max-w-4xl px-4 py-4">
          <div className="mb-2 flex items-center text-sm text-gray-500">
            <button className="flex items-center transition-colors hover:text-[#800000]">
              <ArrowLeft className="mr-2 h-4 w-4" />
              Kembali ke Blog
            </button>
            <ChevronRight className="mx-2 h-4 w-4" />
            <span className="text-[#800000]">{articleData.category}</span>
          </div>
        </div>
      </div>
